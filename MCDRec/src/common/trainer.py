...
#             other_params = [param for name, param in self.model.named_parameters() if not name.startswith('diff')]
#             optimizer = optim.Adam([{'params': other_params, 'lr': self.learning_rate}, {'params': self.model.diff.parameters(), 'lr': self.diff_lr} ])  
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        """
#         self.model.load_state_dict(torch.load('./saved_models/baby_best_model_pthNone_0.8_0.0_freedom_neg_ver10_id.pth'))
        self.model.train()
...
#                     model_save_path = os.path.join('./saved_models/', 'baby_best_model_pth{}_{}_{}_{}_{}.pth'.format(self.config['n_layers'],self.config['dropout'],self.config['reg_weight'], self.config['model'],'id'))
